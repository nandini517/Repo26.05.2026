...
def render_planogram_grid(plan_layout, title, badge):
    ...
    for r in reversed(range(GRID_ROWS)):
        for c in range(GRID_COLS):
            if (r, c) == (0, 0):
                cells_html.append(
                    """
                    <div class='ds-cell ds-dispatch' style='width:34px;height:34px;' title='📦 DISPATCH (0,0)'>
            ...
            if (c + 1) % 4 == 0:
                cells_html.append(
                    "<div class='ds-cell' style='background:#f3f4f6;border-style:dashed;' title='Replenishment aisle'></div>"
                )
                continue
            ...
            if sku is None:
                cells_html.append(
                    "<div class='ds-cell' style='background:#ffffff;border-style:dashed;' title='Empty'></div>"
                )
                continue

            zone_class = (
                "ds-zone-a"
                if sku.velocityTier == "A"
                else "ds-zone-b"
                if sku.velocityTier == "B"
                else "ds-zone-c"
            )
            tooltip = (
                f"{sku.name} | {sku.category} | Orders/day: {sku.ordersPerDay} | "
                f"{sku.velocityTier}-tier"
            )
            cells_html.append(
                f"""
                <div class='ds-cell {zone_class}' title='{tooltip}'>
                  {abbreviate_name(sku.name)}
                  <span class='ds-emoji'>{CATEGORY_EMOJI.get(sku.category,'📦')}</span>
                </div>
                """
            )
    ...
    st.markdown(
        ...
        <div class='ds-card'>
          ...
          <div class="ds-grid" style="grid-template-columns: repeat({GRID_COLS}, 34px);">
            {''.join(cells_html)}
          </div>
    ...
    with tabs[3]:
        ...
        st.markdown(
            ...
            <div class='ds-card' style="margin-top:14px;">
              <div style="font-weight:900;color:{PRIMARY};margin-bottom:8px;">Travel Comparison</div>
              <div style="display:flex;gap:22px;flex-wrap:wrap;">
                <div style="font-size:14px;"><b>Before:</b> {BEFORE_TRAVEL_AVG}m avg travel</div>
                <div style="font-size:14px;"><b>After:</b> {AFTER_TRAVEL_AVG}m avg travel</div>
                <div style="font-size:14px;"><b>Reduction:</b> {reduction_pct:.1f}%</div>
              </div>
              <div style="margin-top:8px;font-size:14px;">
                <b>OPRH:</b> baseline vs +34% improvement &nbsp;&nbsp;|&nbsp;&nbsp;
                <b>Est. Cost per Order:</b> 18% reduction
              </div>
            </div>
            """,
            unsafe_allow_html=True,
